...
namespace TcpServerDemo
{
    ...
    public class SerialProvider : TcpServiceProvider
	{
        ...
        private NotifyIcon msgbaloon = null;

        public SerialProvider(NotifyIcon n)
        {
            msgbaloon = n;
            msgbaloon.BalloonTipTitle = "Tcp2Com";
            msgbaloon.BalloonTipText = "...";
        }
        ...
        {
            return new SerialProvider(msgbaloon);
        }
                ...
                if (com == null)
                {
                    ...
                    if (buffer[0] >= 0 && buffer[0] < ports.Length)
                        try
                        {
                            ...
                            conn = state;
                            com.Open();
                            string s = state.RemoteEndPoint + "  connected to  " + com.PortName;
                            state.Write(Encoding.ASCII.GetBytes(s+"\r\n"), 0, s.Length+2);
                            if (msgbaloon != null)
                            {
                                msgbaloon.BalloonTipText = s;
                                msgbaloon.ShowBalloonTip(30000);
                            }
                        }
                        catch (Exception e)
                        {
		...
		{
            string s = com.PortName + "  disconnected";
            state.Write(Encoding.ASCII.GetBytes(s + "\r\n"), 0, s.Length + 2);
            if (msgbaloon != null)
            {
                msgbaloon.BalloonTipText = s;
