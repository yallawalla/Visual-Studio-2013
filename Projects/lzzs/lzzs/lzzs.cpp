...
short		match_position, match_length,  /* of longest match.  These are
...
void DeleteNode(short p)  /* deletes node p from tree */
{
	short  q;
...
int main(int argc, char *argv[])
{
	...
	if ((s = argv[0], s[1] || strpbrk(s, "zZuU") == NULL)
	 || (s = argv[1], (infile  = fopen(s, "rb")) == NULL)
	 || (s = argv[2], (outfile = fopen(s, "wb")) == NULL)) {
		printf("??? %s\n", s);  return EXIT_FAILURE;
	}
	if (toupper(*argv[0]) == 'Z') Encode();  else Decode();
...
int mainn(array<System::String ^> ^args)
{
