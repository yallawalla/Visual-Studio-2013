...
namespace MATHLIB3D {
	...
	public:
		...
		inline const float GetDot(const Vector3D& vector) const;
		inline const float GetAngle(const Vector3D& vector) const;
		...
		inline const float GetSphericCoordinates(float& angle1, float& angle2) const;
		inline const float GetDistance(Vector3D& vector) const;

		/* Set Methods - Original Vector Modified */
		inline Vector3D& SetMagnitude(const float& magnitude);
		...
		inline Vector3D GetMul(const float& scalar) const;
		inline Vector3D& SetMul(const float& scalar);
		inline Vector3D& SetMul(const Vector3D& vector, const float& scalar);

		inline Vector3D GetDiv(const float& scalar) const;
		inline Vector3D& SetDiv(const float& scalar);
		inline Vector3D& SetDiv(const Vector3D& vector, const float& scalar);
