...
namespace RawEthernet
{
	...
	class RawEthernet
	{
		...
		static void Main(string[] args)
		{
            ...
            {
                byte[] pack = new byte[1500];
                int n = rawether.SlipRead(pack, pack.Length);
                if (n > 0)
                {
                    for (int i = 0; i < n; ++i)
                        Console.Write(" {0:X2}", pack[i]);
                    Console.WriteLine();
                }
            }
		...
		#region METHODS
			...
			{
                return SlipSend(packet, packet.Length);
			}


            private bool SlipSend(byte[] pack, int len)
            {
                byte[] escpack = new byte[1500];
                uint uiSentCount = 0;
                ...
                do
                {
                    ...
                    {
                        escpack[j++] = SLIP_ESC;
                        escpack[j++] = SLIP_ESC_END;
                        continue;
                    }
                    if (pack[i] == SLIP_ESC)
                    {
                        escpack[j++] = SLIP_ESC;
                        escpack[j++] = SLIP_ESC_ESC;
                        continue;
                    }
                    escpack[j++] = pack[i];
                } while (++i < len);
                ...
                unsafe
                {
                    fixed (void* pvPacket = escpack)
                    {
                        packetSent = WriteFile(this.m_iHandle, pvPacket, (uint)j, &uiSentCount, 0);
                    }
                }
            ...
            private int SlipRead(byte[] pack, int len)
            {
                uint uiReadCount = 0;
                bool packetRead;
                unsafe
