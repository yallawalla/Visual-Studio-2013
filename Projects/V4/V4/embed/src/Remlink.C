...
/* comm. status		*/			p = get_int(&j,p);
							...
							break;
/*------------------------------------------------------*/
			case METEO_REQ:	*p++ = METEO_UPDT;					/* klic biltena			*/
							j=sizeof(char);						
							for(i=0; i<8; ++i)
							...
							break;
/*------------------------------------------------------*/
			case METEO_UPDT:p=link_buff;						/* odgovor na klic		*/
							++p;
							for(i=0; i<8; ++i)
							...
							break;
/*------------------------------------------------------*/
			case METEO_TX:	p=link_buff;						/* posiljanje biltena	*/
							++p;
							for(i=0; i<8; ++i)
								p=get_int(&meteo[i][0],p);
							meteo_A=link_source;
							...
							*p++ = METEO_ACK;
							...
							break;
/*------------------------------------------------------*/
			case METEO_ACK: p = link_buff;						/* potrditev sprejema	*/
                			p = get_int(&i,++p);
							p = get_int(&i,p);
							p = get_int(&j,p);
							METEO->ntx=i-1;
							if(METEO->txmode[i-1] != j)
								METEO->txmode[i-1]=j;
							Ungetch(REFRESH,0);
							break;
/*------------------------------------------------------*/
...
/*--------------------------------------------------------*/
double	BpTick=0.06,TickIdle=20;
int		RFAck(int a,char *q,int n)
		{
		DspHold(50);
		n=SendBlk(a,q,n,CTRL_F_UNM);
		DspHold((int)((((8*(n + TickIdle) + 16*abs(key_delay/16)) * (targets_xn(OBP)-1)) * BpTick) + 5));
		return(eof);
		}
/*----------------------------------------------------------*/
