...
/*... loading the library block OBP first, default types must be preloaded !*/ 
			...
			ORG=OBP;					/* default mode EL_AZ_R in local	*/
			d_mode=EL_AZ_R;				/* caka na konec loga				*/
			...
			if(Tamb < 0)				/* zacetni vklop grelca, ce je T<0	*/
			{
				HeatOn();
...
uchar		*GetBlock(uchar *lib)
			...
			while(l != (uchar *)0xffffffff) {
				ll=load_byte(&l,l,sizeof(uchar *));
				if(l != (uchar *)0xffffffff)
					lib = ll;
				}
			return(lib);
			}
...
/*														  */
			...
			l = library;
			WriteGunData(Guns,&l);
			SaveLib(OBP,SaveZone(SaveFonts(SavePar(SavePar(l,0),1))));
#else
...
void		SysMessageLCD_03(void)
			...
			SetTask(SysMessageEnd,50,PIT_task);
			}
...
void		SysMessageLCD_02(void)
			...
			SetTask(SysMessageLCD_03,20,PIT_task);
			}
...
void		SysMessageLCD_01(void)
			...
			SetTask(SysMessageLCD_02,20,PIT_task);
			}
...
void		SysMessageLCD_00(void)
			...
			SetTask(SysMessageLCD_01,20,PIT_task);
			}
...
void		SysMessageLCD(void)
			...
			SetTask(SysMessageLCD_00,100,PIT_task);
