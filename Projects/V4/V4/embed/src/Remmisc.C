...
/*------------------------------------------------------------------*/
int		menu_exe(menu *m,int i,void *vp)
		{
...
coord		*gridto(coord *p,coord *q,int xmode,double units)
			...
			if(z || r)
				...
				el=read_EL()/maxmils*2.0*M_PI;

			if(x || y)
				az=atan2(y,x);
			else
				az=read_AZ()/maxmils*2.0*M_PI;
...
*/							sprintf(p,"%2d.%02d ",ll->xc->t.min,ll->xc->t.sec);
			...
			switch(dmode) {
				case N_E_Z:		sprintf(p,"%6.0f ",round(ll->xc->x,6));
								xyLCD(0,0);
								puts_lcd(p);
								sprintf(p,"%6.0f ",round(ll->xc->y,6));
								xyLCD(0,1);
								puts_lcd(p);
								sprintf(p,"%6.0f ",round(ll->xc->z,6));
								xyLCD(0,2);
								puts_lcd(p);
								break;
				case EL_AZ_R:	if(!NORTH && (ll->xc->y < 0))
									ll->xc->y += mils;
								sprintf(p,"%6.2f ",fmod(round(ll->xc->y,4),mils)/100);
								translate(p,'.','-');
								xyLCD(0,0);
								puts_lcd(p);	
								sprintf(p,"%6.0f ",round(ll->xc->z,5));
								xyLCD(0,1);
								puts_lcd(p);									
								sprintf(p,"%6.2f ",fmod(round(ll->xc->x,4),mils)/100);	
								translate(p,'.','-');
								xyLCD(0,2);
								puts_lcd(p);
								break;
				case AZ_R_dZ:	if(!NORTH && (ll->xc->x < 0))
								...
								xyLCD(0,0);
								puts_lcd(p);	
								sprintf(p,"%6.0f ",round(ll->xc->y,5));
								xyLCD(0,1);
								puts_lcd(p);									
								sprintf(p,"%6.0f ",round(ll->xc->z,5));	
								xyLCD(0,2);
...
struct 	 ParList	{void *a;int s;char *n;char t;} ParList[] =
		 ...
		 {&lcd,sizeof(int),"lcd",2},
		 {&Backlit,sizeof(int),"backlit",2},

		 {&ChEcho,sizeof(int),"echo",3},
		 {&BpTick,sizeof(double),"BpTick",3},
		 {NULL,null,NULL,null}
		 };
/*-------------------------------------------------------------*/
...
/* shaft_ROC417(void)	=> Heidenhain ROC417,17-bit,gray*/
/*														*/
/* modifikacije:										*/
/* V4.0  dodan case za ROC417							*/
/*														*/
/*------------------------------------------------------*/
void	shaft_enable(void)
{
	...
	case NOSHAFT:
		...
		DeleteTask(shaft_ROC417,PIT_task);
	...
	case ROC_G:
		...
		break;
	case ROC417:
		if(!CheckTask(shaft_ROC417,PIT_task))
		{
			SHtoR = 2.0*M_PI/0x8000;
			DeleteTask(shaft_ROC415,PIT_task);
			DeleteTask(shaft_ROC415G,PIT_task);
			SetTask(shaft_ROC417,3,PIT_task);
		}
		break;
	case ROC:
	default:
