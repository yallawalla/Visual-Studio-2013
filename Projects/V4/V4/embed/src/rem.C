...
*/
	...
	TPRAM10(6) = 0x0068;	/* PWMPER = 32768*512/32 /40kHz */
/*	CIER |= 0x0400;			 pwm interrupt enable	V4.0	*/
...
/*----------------------------------------------------------*/
/* defs for TPU Channel 10, usec countdown					*/

volatile	long usecs=0;					

_IH void tpuint_pwm ()						/* V3.0			*/
{						/* 40kHz us countdown  counter		*/
	CISR &= ~0x0400;	/* clear interrupt req.				*/
	if(usecs > 0)
		usecs -= 25;
}
/*----------------------------------------------------------*/
...
// podpora za MP
	...
	while(recvmp((char *)&rxbuff[rxend]))
		rxend=++rxend & RXpc;
//
