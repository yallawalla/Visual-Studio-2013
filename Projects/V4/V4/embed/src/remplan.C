...
/*---  vpis prenosne sekvence v *p    ----------------------*/
		*p++ = COMM_REQ;
		j = sizeof(char);
/*---  stevilska oznaka sekvence      ----------------------*/
		j += put_int(&p,(*cp)->n);
...
/*--- target data                      ---------------------*/	
		j +=tx_lib(&p,tg);
		j += put_rtime(&p,&Time);
		j += put_double(&p,BpTick);
		j += put_double(&p,TickIdle);
		RFCall((*cp)->to,c,j);
/*--- command status                   ---------------------*/
		for(i=0;i<MAXGUN;++i)
			(*cp)->txmode[i]=M_UNDEF;
...
*/			if(Lr->xc)
			{
				...
				else
				{
					...
					for(l->xc=l->x; l->xc; l->xc=l->xc->next)
					{
						...
						l->xc->refp = ScaleAngle(ll->xc->y,maxmils,MR10,eof);
					}
					l->xc=x;
					freelib(ll);
				}
...
/*--- parametri ukaza                  -----------------*/
		...
		p=get_long(&dt,p);
/*	skaniranje cilja															*/
		c->tg=LinkAddr(l=scan_lib(&p));
		if(ll=FindLib(c->tg))
		{
			i=targets_xn(ll)-1; 			/* izravnava xc; da SETUP GUN ne	*/
			for(l->xc=l->x; i && l->xc->next; l->xc=l->xc->next)
				--i;						/* prepise vrstnega stevila orozja	*/
			copylib(l,ll);					/* prepis							*/
			freelib(l);
		}
		else
		{
			if(!Ln)							/* sicer nov vpis v Ln				*/
				Ln=l->next=l;
			else
			{
				l->next=Ln->next;
		...
		p=get_rtime(&Time,p);
		p=get_double(&BpTick,p);
		p=get_double(&TickIdle,p);
...
/* zaokrozanje na 0.5 sec !									*/
		...
		addsecs(&Time,-dt);
		WriteTime(&Time);
/*--- nastavitev rxmode = PENDING !    -----------------*/
 		c->rxmode=M_PEND;
/*--- preveri izbrano orozje na OBP    -----------------*/
		i=targets_xn(OBP);
		j=targets_n(OBP->x);
		c->chk=null;
...
/*----------------------------------------------------------*/
int		list_METEO(void *vp)
{
		char 	c[32],cc[32],ccc[32];
		command **cp = vp;
		int		i,m;
/*... izhod v slucaju, ce ni nalozene balistike			..*/
		if(!Guns)
...
/*........................................................*/
			...
			if(!LinkAddrType(meteo_A))
				...
				LinkAddrText(meteo_A,ccc);

			sprintf(c,"from :%s",ccc);
			puts_lcd(c);
/*........................................................*/
			xyLCD(0,3);
...
*/			sprintf(c,"%s:%.2lf",M_BEARING,GetRefp(l)/100.0);
	...
	case 1:	sprintf(c,"%6.2f",fmod(round(blll->xc->y,4),mils)/100);
			translate(c,'.','-');
			...
			sprintf(c,"%6.2f",fmod(round(blll->xc->x,4),mils)/100);
			translate(c,'.','-');
			if(p)
			...
			{
				sprintf(c,"%6.2f      ",fmod(round(blll->xc->z,4),mils)/100);
				translate(c,'.','-');
			}
			else
				sprintf(c,"            ");
...
double	NaklonMuzz(double);
		...
		if(!corr)
		{								/* brez korekcije => izracun dV0	*/
			q=bll;						/* bll backup						*/
			r=blll;
			bll=blll=NULL;
			corr = -1;					/* preracun cilja na koord. zadetka	*/
			CalcTarget(&CP);			/* naredi se nov bll !!! ~~~		*/
