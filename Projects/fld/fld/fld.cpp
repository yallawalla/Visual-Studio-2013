// fld.cpp : Defines the entry point for the console application.
//
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	int N = 50;
	FluidCube *f = FluidCubeCreate(N, 10, 10, 0.001);
	FluidCubeAddDensity(f, N / 2, N / 2, N / 2, 5);
	FluidCubeAddVelocity(f, N / 2, N / 2, N / 2, 10,10,10);
	for (int i = 0; i < 20; ++i) {
		FluidCubeStep(f);
		system("CLS");
		for (int x = 0; x < N; ++x) {
			for (int y = 0; y < N; ++y) {
				SetConsoleTextAttribute(hConsole, f->density[IX(x, y, N / 2)] * 10000);
				printf("--");
			}
				printf("\r\n");
		}
